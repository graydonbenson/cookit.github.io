...
function reloadCart() {
  const cartContent = document.querySelector(".fridgeSidebar-content");
  ...
  cartList.forEach((value, key) => {
    ...
    if (value != null) {
      ...
      cartShopBox.classList.add("fridgeSidebar-box");

      //build html structure to display item
      //add the item data to cart list and show it
      cartShopBox.innerHTML = `
            <img class="fridgeSidebar-img" src="${value.product_image}" alt="${
        value.product_name
      }">
            <div class="detail-box">
                <div class="fridgeSidebar-product-title-product-title">${
                  value.product_name
                }</div>
                <div class="product-quantity">
                    <button class="more-quantity" onclick="changeQuantity(${key}, ${
        value.quantity - 1
      })"></button>
                    <span> 3 </span>
                    <button class="less-quantity" onclick="changeQuantity(${key}, ${
        value.quantity + 1
      })"></button>
                </div>
            </div>
            <div class="fridgeSidebar-remove">
            <button class="remove_Button" onclick="removeCartItem(event, ${key})">Remove</button></div>
...
document.addEventListener("DOMContentLoaded", function () {
  const cart = document.querySelector(".fridgeSidebar");
  const cartIcon = document.querySelector(".material-symbols-outlined");
  const closeButton = document.getElementById("close-fridgeSidebar");
  const searchStoreContainer = document.querySelector(
    ".search-store__container"
  );
  ...
  function pushProductList() {
    ...
    <button class="add-fridge" onclick="addToCart(${key})">
        Add to Fridge
    </button>`;
  ...
  function performSearch() {
    ...
    if (searchText.trim() === "") {
      ...
      // Otherwise, filter items that include the search text and display them
  ...
  function displayResults(results) {
    ...
    // If there are no results and the search input is empty, keep the results hidden
    if (results.length === 0 || searchInput.value.trim() === "") {
      resultsContainer.classList.add("hidden");
    } else {
      // There are results, remove the hidden class to show them
  ...
  function toggleCheckout() {
    if (cartContent.style.display === "block") {
      ...
      checkoutButton.textContent = "View Fridge";
    } else {
